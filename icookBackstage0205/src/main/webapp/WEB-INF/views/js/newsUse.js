...
let productInfoTest = 	`倉庫租月即將到期不再續約, 商品出清大特賣<br>
						即日起所有商品1折起!!<br>
						要買要快!!!!
						`;
...
//一鍵輸入
function speedKeyIn(){
	$("#titleIn").val("租約到期!!! 商品出清一折起!!!");
	CKEDITOR.instances["newsContentIn"].setData(productInfoTest);
}
			...
			if(json.newsPageJson != null){
				...
				for(let i=0 ; i<newsPageJson.length ; ++i){
					tableContant	+= 	"<tr><td>" + newsPageJson[i].newsId
									+	"<td class='title123'>" + newsPageJson[i].title
